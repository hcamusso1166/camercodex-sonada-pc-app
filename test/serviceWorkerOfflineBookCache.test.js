...
test('actualización v19→v20 precarga un índice nuevo y preserva las tres cachés BTI', async () => {
  const oldName = 'camer-codex-cache-v19';
  const bookIds = [
  ...
  await dispatchInstall(listeners);
  assert.deepEqual(calls.opened, ['camer-codex-cache-v20']);
  const fresh = cacheMap.get('camer-codex-cache-v20');
